...
Tài liệu này phân tích các ràng buộc `onDelete` trong schema Prisma và đưa ra các đề xuất cải tiến để đảm bảo tính toàn
vẹn dữ liệu và hành vi xóa phù hợp.
...
- Sử dụng `SetNull` vì đây là quan hệ optional, khi category bị xóa, budget vẫn có thể tồn tại nhưng không còn category
  cụ thể
...
- Hoặc thay đổi thành `SetNull` và cho phép categoryId nullable nếu muốn linh hoạt hơn (nhưng cần cập nhật business
  logic)
